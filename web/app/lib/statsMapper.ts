import type { LifeStats, PhraseResult, SourceCardView, SourceStatView } from "./types";
...
export function mapSourceCards(sources: string[], stats: LifeStats, phrases: PhraseResult[]): SourceCardView[] {
    const resolvedSources = resolveDisplaySources(sources, stats);
    const phraseBySource = buildPhraseBySource(phrases);
    ...
    return resolvedSources.map((source) => {
        ...
        const phrase = phraseBySource[source] ?? "Il tuo anno in dati.";
...
function buildPhraseBySource(phrases: PhraseResult[]): Record<string, string> {
